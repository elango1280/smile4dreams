...
function NLP_Captcha()
{
    $captcha=  "<script type=\"text/javascript\"> var NLPOptions = {key:'".NLPCAPTCHA_PUBLISHER_KEY."'};</script>";
...
function NLP_Validate($nlpIdentifier,$nlpAnswer){
		...
		if(NLPCAPTCHA_VALIDATE_KEY==NULL or trim(NLPCAPTCHA_VALIDATE_KEY) == '' or strlen(trim(NLPCAPTCHA_VALIDATE_KEY))<>32)
		{
		...
		if((empty($nlpAnswer)||empty($nlpIdentifier))){
			return false;
                ...
                $arr_params=array('ValidateKey'=>NLPCAPTCHA_VALIDATE_KEY, 'Identifier'=>$nlpIdentifier,'Answer'=>$nlpAnswer);
